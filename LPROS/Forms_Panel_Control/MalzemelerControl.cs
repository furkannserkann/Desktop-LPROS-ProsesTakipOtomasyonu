    ...
    public partial class MalzemelerControl : Form
    {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            { 
                ...
                {
                    Items.panelMalzemeler.dataGridview.DataSource = Sc.GET_DATATABLE(SqlConnector.TableMalzemeler + " and fiyat<=@parametre1", new string[] { textBox4.Text});
                }
                else if (textBox1.Text != "" && textBox4.Text == "")
                {
                    Items.panelMalzemeler.dataGridview.DataSource = Sc.GET_DATATABLE(SqlConnector.TableMalzemeler + " and fiyat>=@parametre1", new string[] { textBox1.Text });
                }
                else if (textBox4.Text != "" && textBox1.Text != "" && Convert.ToInt32(textBox1.Text) <= Convert.ToInt32(textBox4.Text))
                {
                    Items.panelMalzemeler.dataGridview.DataSource = Sc.GET_DATATABLE(SqlConnector.TableMalzemeler + " and fiyat BETWEEN @parametre1 AND @parametre2", new string[] { textBox1.Text, textBox4.Text });
                }
            }
            else if (textBox5.Text != "")
            {
                if (textBox4.Text != "")
                {
                    Items.panelMalzemeler.dataGridview.DataSource = Sc.GET_DATATABLE(SqlConnector.TableMalzemeler + " and  fiyat<=@parametre1 AND adi like '%'+@parametre2+'%' ", new string[] { textBox4.Text, textBox5.Text});
                }
                else if (textBox1.Text != "")
                {
                    Items.panelMalzemeler.dataGridview.DataSource = Sc.GET_DATATABLE(SqlConnector.TableMalzemeler + " and fiyat>=@parametre1  AND adi like '%'+@parametre2+'%'", new string[] { textBox1.Text, textBox5.Text });
                }
                else if (textBox4.Text != "" && textBox1.Text != "" && Convert.ToInt16(textBox1.Text) < Convert.ToInt16(textBox4.Text))
                {
                    Items.panelMalzemeler.dataGridview.DataSource = Sc.GET_DATATABLE(SqlConnector.TableMalzemeler + " and adi like '%'+@parametre1+'%' AND fiyat BETWEEN @parametre2 AND @parametre3", new string[] { textBox5.Text, textBox1.Text,textBox4.Text });
                }
                else
                {
                    Items.panelMalzemeler.dataGridview.DataSource = Sc.GET_DATATABLE(SqlConnector.TableMalzemeler + " and adi like '%'+@parametre1+'%'", new string[] { textBox5.Text });
                }
            }
        }
